...
raw Help Scout prose never reaches the model. Normal target intake returns readable masked customer/staff messages in order; identities, secrets, URLs, quoted history, signatures, notes, attachments, subject, preview, people metadata, and raw IDs are not exposed. The target transcript is evidence for forming a sanitized question and closed claims, not safe query text by itself. Typed-facts fallback is quarantine and never a reason to widen source access. Historical tickets remain typed-facts-only; only an actual closed safe public term may form history queries. `blocked` stops the workflow safely without source-derived content.
