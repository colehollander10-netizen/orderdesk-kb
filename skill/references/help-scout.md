...
Do not put it—or any customer, order, store, email, domain, URL, or other raw identifier—into a history or public-KB query. For Help Scout history, pass one or two structured `terms` entries whose `field` is exactly `subject`, `body`, or `tag`; never pass raw Help Scout query syntax or operators. Stop on policy, scope, masking, audit, or source-read failure. Never create, save, or send a Help Scout draft.
