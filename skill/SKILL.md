...
A named real Help Scout ticket is the intake artifact. Proceed without a second approval prompt. Verify that `helpscout_status` advertises both `helpscout.support-context.typed-facts.v1` and `helpscout.support-context.complete-target.v1` before calling `helpscout_get_support_context`; an absent capability is a technical blocker. The bridge must inspect the complete target conversation across all provider pages without per-message truncation while returning typed facts only. Before accepting or passing a correlation handle, require `helpscout.support-correlation.opaque-handle.v1` with output mode `opaque-correlation-envelope`; otherwise ignore correlation fields and mark correlation unavailable.
