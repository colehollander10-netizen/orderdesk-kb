...
def evaluate_runtime_contract(payload: Any) -> dict[str, bool | str]:
    ...
    if type(capabilities) is not list or type(tools) is not list:
        return RUNTIME_CONTRACT_MISMATCH.copy()
    if (
        ...
        return RUNTIME_CONTRACT_MISMATCH.copy()
    if any(type(value) is not str for value in capabilities + tools):
        return RUNTIME_CONTRACT_MISMATCH.copy()
    if len(capabilities) != len(set(capabilities)) or len(tools) != len(set(tools)):
        return RUNTIME_CONTRACT_MISMATCH.copy()
    if (
